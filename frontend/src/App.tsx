...
export function App() {
  ...
  const authSession = useRef<Session | null>(null)
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, next) => {
      ...
      if (authSession.current?.user.id !== next?.user.id) {
        generation.current++
        reads.current++
  ...
  useEffect(() => {
    ...
    async function load(initial = false) {
      ...
      const attempt = ++reads.current
      if (initial) setLoading(true)
      try {
        const params = new URLSearchParams({ timezone })
        if (selected) params.set('selected', selected)
        const response = await fetch(`/api/calendar?${params}`, {
          headers: { Authorization: `Bearer ${authSession.current?.access_token}` }, cache: 'no-store',
        ...
        if (generation.current === version && reads.current === attempt && !mutation.current) { setCalendar(data); setMessage('') }
      } catch (error) {
        if (!controller.signal.aborted && generation.current === version && reads.current === attempt && !mutation.current) {
          setMessage(error instanceof Error && error.name === 'TimeoutError' ? 'Loading took too long. Please retry.' : error instanceof TypeError ? 'Could not connect. Check your connection and retry.' : error instanceof Error ? error.message : 'Could not load this day.')
        }
  ...
  function chooseDay(day: string) {
    ...
    document.getElementById("habits-title")?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? "auto" : "smooth", block: "start" })
    generation.current++
    setSelected(day)
    setCalendar(null)
    setName('')
