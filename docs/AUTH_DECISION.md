...
Supabase manages browser authentication without a custom server session store. Browser token storage still requires strong XSS prevention: React renders habit names as text, and no raw HTML injection is used. Session data is cleared from the UI on auth state changes, and in-flight responses are ignored after account changes. A deployment-specific Content Security Policy and browser checks are release requirements.
